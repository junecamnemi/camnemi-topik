...
const APP = {
  ...
  mockIdx: 0,
  mockAnswers: {},
  dailyMocks: [],           // today's AI-generated mocks (TOPIK I + II) from /api/daily-mocks
...
const T = {
  en: {
    ...
    home_ai_read: 'AI Reading questions', home_ai_listen: 'AI Listening questions', home_ai_quick: 'Quick test', home_ai_redo: 'AI Redo', home_ai_redo_sub: '5 similar problems from your wrong & saved questions',
  ...
  ko: {
    ...
    home_ai_read: 'AI 리딩 문제', home_ai_listen: 'AI 리스닝 문제', home_ai_quick: '빠른 테스트', home_ai_redo: 'AI Redo', home_ai_redo_sub: '틀린·북마크 문제로 유사문제 5개 만들기',
  ...
  km: {
    ...
    home_ai_read: 'សំណួរអានដោយ AI', home_ai_listen: 'សំណួរស្តាប់ដោយ AI', home_ai_quick: 'តេស្តរហ័ស', home_ai_redo: 'AI Redo', home_ai_redo_sub: 'សំណួរស្រដៀង ៥ ពីចម្លើយខុស & ចំណាំ',
...
/* Resolve a mock test's questions. Daily AI mocks embed their own questions[];
   the static MOCK_TESTS reference bank ids via qids. */
function mockQuestions(m) {
  if (!m) return [];
  if (Array.isArray(m.questions) && m.questions.length) return m.questions.filter(Boolean);
  return (m.qids || []).map(qById).filter(Boolean);
}

/* ---------- Question bank: AI-generated accumulation + no-repeat ----------
   A growing pool of AI-written questions is kept in localStorage (LS.aibank),
   merged into the question universe so daily/redo/section can draw fresh items.
   Any question the user has already answered (tracked in LS.progress) is treated
   as "solved" and is filtered out of fresh sets. */
function _bankFreshQids() {
  // qids the user has answered before → never offered again
  const solved = new Set();
  Object.keys(lsGet(LS.progress, {})).forEach(id => solved.add(id));
  // also qids recorded in daily/section history as done
  try { Object.values(lsGet(LS.daily, {})).forEach(d => Object.keys(d.done || {}).forEach(id => solved.add(id))); } catch (e) {}
  try { Object.values(lsGet(LS.section, {})).forEach(d => Object.keys(d.done || {}).forEach(id => solved.add(id))); } catch (e) {}
  return solved;
}
/* All AI-generated questions accumulated so far (from LS.aibank), deduped by id. */
function aiBankQuestions() {
  const arr = lsGet(LS.aibank, []);
  const seen = new Set(); const out = [];
  (Array.isArray(arr) ? arr : []).forEach(q => {
    if (!q || !q.id || seen.has(q.id)) return;
    // only keep questions that still have a usable correct answer
    if (q.section === 'writing') { if (q.answerModel) { seen.add(q.id); out.push(q); } return; }
    if (Array.isArray(q.options) && q.options.length >= 2 && q.correct != null) { seen.add(q.id); out.push(q); }
  });
  return out;
}
/* Merge the AI bank into the main universe. Callers that want fresh questions use
   freshQuestionPool(level, section, type). */
function allQuestionsWithAI() {
  return allQuestions().concat(aiBankQuestions());
}
/* Return questions the user has NOT solved yet, preferring the AI bank first so the
   accumulated AI questions actually get used; falls back to the static bank if thin. */
function freshQuestionPool(opts) {
  opts = opts || {};
  const solved = _bankFreshQids();
  const wantLevel = opts.level;
  const lvlOk = q => !wantLevel || levelOf(q) === wantLevel || q.level === wantLevel;
  const secOk = q => !opts.section || q.section === opts.section;
  const typeOk = q => !opts.type || (q.type || q.section) === opts.type;
  const keep = q => lvlOk(q) && secOk(q) && typeOk(q);
  const ai = aiBankQuestions().filter(q => keep(q) && !solved.has(q.id));
  const stat = allQuestions().filter(q => keep(q) && !solved.has(q.id));
  // prefer AI bank (fresh written items), then static; if both too thin, allow solved again
  const comb = ai.concat(stat);
  const fresh = comb.filter(q => !solved.has(q.id));
  if (fresh.length >= (opts.need || 0)) return fresh;
  return comb.length ? comb : allQuestions().filter(q => keep(q));
}
/* Record a freshly AI-generated question into the accumulating bank. */
function addToAIBank(q) {
  if (!q || !q.id) return;
  const arr = lsGet(LS.aibank, []);
  const seen = new Set(arr.map(x => x && x.id));
  if (seen.has(q.id)) return;
  arr.push(q);
  lsSet(LS.aibank, arr.slice(-2000)); // cap to last 2000
}
...
function viewHome() {
  ...
  return `
    ...
    <div class="hm-main-msg">
      <div class="hmm-title"><span class="hmm-a">Study Korean with Your Idol</span></div>
...
async function startSection(sec, lv, type) {
  ...
  // ---- Daily AI vocab set (20 questions generated by the cron) takes priority ----
  if (type === 'vocab') {
    try {
      const seen0 = Object.keys(lsGet(LS.progress, {})).join(',');
      const r0 = await fetch(aiUrl('/daily-vocab?exclude=' + seen0), { signal: AbortSignal.timeout(6000) });
      const d0 = await r0.json();
      if (d0.questions && d0.questions.length >= 5) {
        const qs0 = d0.questions.slice(0, 20);
        APP.section = sec; APP.sectionLevel = target; APP.sectionType = 'vocab';
        APP.sectionQs = qs0; APP.sectionIdx = 0; APP.sectionAnswers = {}; APP.sectionDone = false;
        APP.sectionStart = Date.now();
        const all0 = lsGet(LS.section, {}); all0['reading:' + target + ':vocab'] = { qids: qs0.map(q => q.id), done: {}, level: target, type: 'vocab' };
        lsSet(LS.section, all0);
        go(sec);
        return;
      }
    } catch (e) { /* offline / no backend → build from VOCAB_BANK below */ }
  }
...
function viewMy() {
  ...
  const myOverlay = `
    <div style="display:flex;justify-content:space-between;align-items:center;width:100%;height:100%;gap:10px;">
      <div style="display:flex;flex-direction:column;justify-content:center;gap:8px;text-align:left;">
        <div style="display:flex;align-items:baseline;gap:6px;"><b style="font-size:24px;font-weight:900;line-height:1;">${streak.count}</b><span style="font-size:11px;font-weight:700;opacity:.92;">${t('menu_streak')}</span></div>
        <div style="display:flex;align-items:baseline;gap:6px;"><b style="font-size:24px;font-weight:900;line-height:1;">${acc}%</b><span style="font-size:11px;font-weight:700;opacity:.92;">${t('menu_acc')}</span></div>
      ...
      <div style="display:flex;flex-direction:column;align-items:flex-end;justify-content:center;gap:6px;text-align:right;max-width:52%;">
        ...
        <a class="btn btn-primary btn-sm" style="pointer-events:auto;padding:6px 12px;font-size:12px;white-space:nowrap;" href="login.html">${t('menu_login')}</a>
        ${authed ? `<button class="btn btn-ghost btn-sm" style="pointer-events:auto;padding:6px 10px;font-size:12px;border-color:rgba(255,255,255,.4);color:#fff;" onclick="doLogout()">${t('menu_logout')}</button>` : ''}
      </div>
    </div>`;
  // ---- My Characters: the unlocked characters you've collected (locked = dimmed) ----
    ...
    <div class="app-card my-chars-card ${_charsOpen ? '' : 'collapsed'}" id="my-chars-card">
      <div class="my-chars-grid">
        ${_allChars.map(c => {
          ...
          if (!unlocked) return `<span class="my-char locked" title="${esc(c.name)}"><img src="${charFace(c)}" alt="${esc(c.name)}" loading="lazy"><span class="my-char-lock">🔒</span></span>`;
          return `<button class="my-char ${c.id===_curId?'on':''}" onclick="viewCharProfile('${c.id}')" title="${esc(c.name)}">
            <img src="${charFace(c)}" alt="${esc(c.name)}" loading="lazy">
            ${c.id===_curId?`<span class="my-char-cur">✓</span>`:''}
          </button>`;
        }).join('')}
      </div>
    </div>`;
...
async function gradeWriting(area) {
  ...
  const q = area === 'daily' ? APP.daily[APP.dailyIdx] : mockQuestions(APP.mock)[APP.mockIdx];
...
/* ================= MOCK TEST ================= */
/* ---- daily AI mocks (generated every day by the cron; served by /api/daily-mocks) ---- */
function allMocks() { return [].concat(APP.dailyMocks || [], window.MOCK_TESTS || []); }
async function loadDailyMocks() {
  if (APP.dailyMocks && APP.dailyMocks.length) return APP.dailyMocks;
  try {
    const r = await fetch(aiUrl('/daily-mocks'), { signal: AbortSignal.timeout(6000) });
    const d = await r.json();
    if (Array.isArray(d.mocks)) APP.dailyMocks = d.mocks;
  } catch (e) { /* offline / no backend → static MOCK_TESTS only */ }
  return APP.dailyMocks || [];
}
/* "Today's questions have arrived" — in-app banner + optional browser notification.
   Fires at most once per day, only when today's generated content is actually present. */
function checkDailyArrival() {
  try {
    const today = todayStr();
    const mocks = APP.dailyMocks || [];
    const hasToday = mocks.some(m => m.date === today);
    if (!hasToday) return;
    if (lsGet(LS.dailyNotified, '') === today) return;
    lsSet(LS.dailyNotified, today);
    // in-app banner
    if (!document.getElementById('daily-arrive-banner')) {
      const canAsk = (window.Notification && Notification.permission === 'default');
      const b = document.createElement('div');
      b.id = 'daily-arrive-banner';
      b.className = 'daily-arrive-banner';
      b.innerHTML = `<span class="dab-ico">🔔</span>
        <span class="dab-t">${LANG === 'ko' ? '오늘의 문제가 도착했어요!' : LANG === 'km' ? 'សំណួរថ្ងៃនេះបានមកដល់ហើយ!' : "Today's questions have arrived!"}</span>
        ${canAsk ? `<button class="dab-bell" onclick="enableDailyNotifications()" title="${LANG==='ko'?'알림 켜기':'Enable notifications'}">🔔+</button>` : ''}
        <button class="dab-go" onclick="dismissDailyBanner();go('mock')">${LANG === 'ko' ? '풀기' : 'Start'}</button>
        <button class="dab-x" onclick="dismissDailyBanner()" aria-label="close">✕</button>`;
      document.body.appendChild(b);
      requestAnimationFrame(() => b.classList.add('show'));
    }
    // browser notification (only if the user already granted permission — never prompt here)
    try {
      if (window.Notification && Notification.permission === 'granted') {
        new Notification('Glowsis AI TOPIK', {
          body: LANG === 'ko' ? '오늘의 새 문제가 도착했어요! 지금 풀어보세요 🎧📖' : "Today's new questions are ready — start now! 🎧📖",
...
function dismissDailyBanner() {
  const b = document.getElementById('daily-arrive-banner');
  if (!b) return;
  b.classList.remove('show');
  setTimeout(() => { try { b.remove(); } catch (e) {} }, 300);
}
/* Opt-in: ask for browser-notification permission so future daily drops can notify. */
async function enableDailyNotifications() {
  try {
    if (!window.Notification) return;
    const p = await Notification.requestPermission();
    if (p === 'granted') {
      new Notification('Glowsis AI TOPIK', { body: LANG === 'ko' ? '알림이 켜졌어요! 매일 새 문제를 알려드릴게요 🔔' : "Notifications on! We'll ping you each day 🔔", icon: 'icons/icon-192.png' });
      toast && toast(LANG === 'ko' ? '🔔 알림이 켜졌어요!' : '🔔 Notifications enabled!');
    }
  } catch (e) {}
}
function viewMock() {
  ...
  return `
    ...
    ${mocks.map(m => `
      <div class="mock-item">
        <div style="flex:1;min-width:0;">
          <div class="mock-name">${m.daily ? `<span class="mock-today">${LANG==='ko'?'오늘':'TODAY'}</span> ` : ''}${esc(m.name)}</div>
...
function startMock(id) {
  const m = allMocks().find(x => x.id === id);
  if (!m) return;
...
function pickMock(i) {
  const q = mockQuestions(APP.mock)[APP.mockIdx];
  if (!q) return;
...
function navMock(d) {
  const qs = mockQuestions(APP.mock);
...
function rankListHTML() {
  ...
  let visible = rows.slice(0, SHOW).map((r, i) => `
    <div class="rk-row ${r.me ? 'me' : ''}">
      <span class="rk-pos">${rankLightstick(i + 1)}</span>
...
/* Read-only full question card (passage + question + all options with the answer
   highlighted + correct answer/explanation). Used by the wrong-answers list and the
   bookmarked-questions list so learners see the full item, not just Q+answer. */
function reviewCard(q, opts) {
  opts = opts || {};
  const letters = '①②③④⑤';
  const head = `<div class="row"><span class="q-num">${esc(q.id)}</span>
    <span class="q-type">${q.section === 'reading' ? '📖' : q.section === 'listening' ? '🎧' : '✍️'} ${esc(typeLabel(q.type))}</span>
    ${q.level ? `<span class="q-level" style="font-weight:800;font-size:11px;padding:1px 7px;border-radius:999px;background:var(--ios-fill);color:var(--ios-label);">L${q.level}</span>` : ''}
  ...
  if (q.section === 'writing') {
    optsHtml = q.answerModel
      ? `<div class="q-explain show" style="margin-top:6px;"><b>${LANG==='ko'?'모범 답안':'Model answer'}</b> — ${esc(q.answerModel)}</div>`
      : '';
  } else if (q.options && q.options.length) {
    optsHtml = q.options.map((o, i) => `
      <div class="q-opt" style="display:flex;align-items:center;gap:8px;border:1.5px solid ${i === q.correct ? '#34d399' : 'var(--border)'};background:${i === q.correct ? 'rgba(52,211,153,.10)' : 'transparent'};">
        <span style="font-weight:700;">${letters[i] || (i + 1)}</span>${esc(o.t)}${o.gl ? `<span class="opt-gloss"> · ${esc(o.gl)}</span>` : ''}
        ${i === q.correct ? `<span style="margin-left:auto;color:#059669;font-weight:800;font-size:11px;">✓</span>` : ''}
      </div>`).join('');
  }
  const explain = (q.explain ? `<div class="q-explain show" style="margin-top:8px;">
      <b>${q.correct !== undefined ? (LANG==='ko'?'✓ 정답: ':'✓ Answer: ') + (letters[q.correct] || '') : ''}</b>
      ${esc(q.explain)}
    </div>` : '');
...
/* Group review items (wrong / bookmarked qids) by TYPE → LEVEL into collapsible
   <details> accordions. Returns { types:[...], html }. */
function groupedReviewItems(qids, opts) {
  opts = opts || {};
  const ko = LANG === 'ko', km = LANG === 'km';
  const L = (a,b,c)=> km?c:(ko?a:b);
  // resolve + group
  const byType = {};  // type -> { qs:[q], lv:{level:[q]} }
  const order = [];
  (qids||[]).forEach(qid => {
    const q = qById(qid);
    if (!q) return;
    const t = q.type || q.section || 'reading';
    if (!byType[t]) { byType[t] = { qs: [], lv: {} }; order.push(t); }
    byType[t].qs.push(q);
    const lv = q.level || 1;
    if (!byType[t].lv[lv]) byType[t].lv[lv] = [];
    byType[t].lv[lv].push(q);
  });
  // sort types by count desc, levels asc
  order.sort((a,b)=> byType[b].qs.length - byType[a].qs.length);
  const typeHtml = [];
  const TYPE_NOTE = {
    reading: ()=>L('지문을 읽고 푸는 문제예요.','Reading-passage comprehension.',''),
    listening: ()=>L('듣고 푸는 문제예요.','Audio-listening questions.',''),
    writing: ()=>L('쓰기 문제예요.','Writing questions.',''),
    vocab: ()=>L('어휘·단어 문제예요.','Vocabulary questions.',''),
    grammar: ()=>L('문법 문제예요.','Grammar questions.','')
  };
  order.forEach(t => {
    const grp = byType[t];
    const icon = t === 'listening' ? '🎧' : (grp.qs[0].section === 'writing' || String(t).startsWith('writing') ? '✍️' : '📖');
    const desc = (TYPE_NOTE[t]||TYPE_NOTE.reading)();
    const lvKeys = Object.keys(grp.lv).map(Number).sort((a,b)=>a-b);
    const lvHtml = lvKeys.map((lv,li)=> {
      const cards = grp.lv[lv].map(q=> reviewCard(q, opts)).join('');
      const lvTag = lv<=2 ? 'L'+lv : 'TOPIK II · L'+lv;
      const open = li===0 ? ' open' : '';
      return `<details class="rw-lv"${open}><summary><span class="rw-lv-l">${lv<=2?'★'.repeat(lv):'L'+lv}</span>
        <span class="rw-lv-n">${grp.lv[lv].length} ${L('개','q','')}</span></summary><div class="rw-lv-body">${cards}</div></details>`;
    }).join('');
    typeHtml.push(`<details class="rw-type" open><summary>
      <span class="rw-ico">${icon}</span>
      <span class="rw-tname">${esc(typeLabel(t))}</span>
      <span class="rw-count">${grp.qs.length}</span>
      <span class="rw-desc">${desc}</span>
    </summary><div class="rw-type-body">${lvHtml}</div></details>`);
  });
...
function viewProgress() {
  ...
  // ---- Study Time dashboard (same weekly calendar as the Home Study Time, so
  // both always agree) + today's tasks / focus sessions / subjects ----
  ...
  const dash = `
    <div class="sec-h"><h2>${ic('schedule',15)} ${t('prog_study_time')}</h2><span class="sub">${t('prog_week')}</span></div>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  applyCharTheme();
  refreshUserBtn();
  // fetch today's AI mocks + fire the "today's set arrived" notification
  loadDailyMocks().then(() => { checkDailyArrival(); if (APP.tab === 'mock') render(); });
