...
function viewHome() {
  ...
  return `
    <div class="app-card elevated big-cta">
      ...
      <h2 style="font-size:22px;color:var(--m3-on-surface);margin-bottom:6px;">${APP.level === 'II' ? 'TOPIK II · Level 3' : 'TOPIK I · Level 1–2'}</h2>
    ...
    <div class="app-card filled">
      <b style="font-size:13px;color:var(--m3-primary);">유형별</b>
      ${acc.byType.length
    ...
    <div class="app-card filled">
      <b style="font-size:13px;color:var(--m3-primary);">난이도별</b>
      ${acc.byLevel.length
    ...
    <div class="app-card elevated"><div class="row" onclick="go('schedule')" style="cursor:pointer;">
      <div><b>🗓 TOPIK 시험 일정</b><div class="sub">나라별 시험일 · 접수기간 · 결과 발표</div></div><span style="color:var(--m3-secondary);">→</span></div></div>
    <div class="app-card elevated"><div class="row" onclick="go('learn')" style="cursor:pointer;">
      <div><b>📚 Weak-spot review</b><div class="sub">Vocab &amp; grammar based on your mistakes</div></div><span style="color:var(--m3-secondary);">→</span></div></div>
    <div class="app-card elevated"><div class="row" onclick="go('wrong')" style="cursor:pointer;">
      <div><b>📓 Wrong answer notes</b><div class="sub">${lsGet(LS.wrong, []).length} recent misses · type-wise breakdown</div></div><span style="color:var(--m3-secondary);">→</span></div></div>
  `;
}
  ...
  return `
    <div class="app-card">
      ...
      <div style="margin:8px 0;">
        ${isAI
          ? '<span style="font-size:11px;color:var(--m3-secondary);font-weight:800;">✨ Generated by AI — unique to you</span>'
...
function viewDailyResult() {
  ...
  return `
    <div class="app-card big-cta">
      <div class="cta-ico">🎯</div>
      <h2 style="font-size:22px;color:var(--m3-on-surface);margin-bottom:6px;">Daily 10 완료!</h2>
      <p class="sub">${r.total}문항 중 ${r.correct + r.wrong}문항 답변</p>
      <div style="margin:18px 0;">
        <div style="font-size:15px;color:var(--m3-on-surface-variant);">나의 예상 점수</div>
        <div style="font-size:52px;font-weight:800;color:var(--m3-primary);">${est.score}<span style="font-size:20px;color:var(--m3-on-surface-variant);"> / ${est.maxScore}</span></div>
        ...
        <div style="margin-top:16px;padding:12px;background:#f0f3fa;border-radius:12px;">
          <div style="font-size:13px;color:var(--m3-on-surface-variant);margin-bottom:6px;">${est.passLabel} · ${passTarget}</div>
          <div style="font-size:30px;font-weight:800;color:var(--m3-secondary);">${APP.level === 'I' ? est.passI : est.passII}%</div>
          <div style="font-size:12px;color:var(--m3-on-surface-variant);margin-top:4px;">TOPIK ${APP.level === 'I' ? 'II' : 'I'} 전환 시 ${APP.level === 'I' ? est.passII : est.passI}%</div>
        </div>
      </div>
      <div class="stat-row">
        <div class="stat-box"><b style="color:var(--m3-secondary);">${r.correct}</b><span>✓ 정답</span></div>
        <div class="stat-box"><b style="color:var(--m3-error);">${r.wrong}</b><span>✗ 오답</span></div>
...
function accBar(label, p, sub) {
  const color = p >= 70 ? 'var(--m3-secondary)' : p >= 40 ? 'var(--m3-tertiary)' : 'var(--m3-error)';
  return `
...
function viewWrong() {
  ...
  return `
    <div class="sec-h"><h2>📊 평균 정답률</h2><span class="sub">전체 ${acc.overall}%</span></div>
    <div class="app-card">
      <b style="font-size:13px;color:var(--m3-primary);">유형별 평균 정답률</b>
    ...
    <div class="app-card">
      <b style="font-size:13px;color:var(--m3-primary);">난이도별 평균 정답률</b>
      ...
      <div class="app-card">
        ${LEARN_CONTENT.grammar.map(g => `
          <div style="padding:8px 0;border-bottom:1px solid var(--border);">
            <b style="color:var(--m3-primary);">${esc(g.k)}</b> <span class="sub">· ${esc(g.t)}</span>
    ...
    ${weakVocab || wrong.length === 0 ? `<div class="sec-h"><h2>🗂 Vocabulary</h2></div>
      <div class="app-card">
        ${LEARN_CONTENT.vocab.map(v => `
          <div style="padding:8px 0;border-bottom:1px solid var(--border);">
            <b style="color:var(--m3-secondary);">${esc(v.k)}</b> <span class="sub">· ${esc(v.t)}</span>
  ...
  return `
    ...
    <div class="sec-h"><h2>📅 달력 보기</h2>
      <span style="display:flex;gap:6px;">
        ...
        <span style="font-size:13px;font-weight:800;color:var(--m3-primary);">${calMonth.slice(0,4)}년 ${+calMonth.slice(5,7)}월</span>
        <button class="btn btn-ghost btn-sm" onclick="scheduleMonth(1)">▶</button>
      </span>
    </div>
    <div class="app-card">
      ...
      return `
        <div class="app-card" style="${st.key === 'done' ? 'opacity:.5;' : ''}">
          <div class="row">
            ...
            <span style="font-size:12px;font-weight:800;color:${st.key === 'reg_ing' ? 'var(--m3-secondary)' : st.key === 'test_wait' ? '#c78a00' : 'var(--m3-on-surface-variant)'};">${statusTxt}</span>
          </div>
          <div style="font-size:16px;font-weight:800;color:var(--m3-on-surface);margin:8px 0 4px;">
            ${d[1]}월 ${d[2]}일 (${dow}) <span style="font-size:12px;color:var(--m3-on-surface-variant);font-weight:600;">${p.date.slice(0,4)}</span>
          </div>
          <div class="sub" style="font-size:12.5px;margin:3px 0;">🖥 접수: <b>${p.reg}</b> ${reg && st.key !== 'done' && st.key !== 'result_wait' ? `<span style="color:var(--m3-primary);font-weight:700;">(${ddayStr(reg.end)})</span>` : ''}</div>
          <div class="sub" style="font-size:12.5px;margin:3px 0;">✏️ 시험: <b>${p.date}</b> ${st.key !== 'done' && st.key !== 'result_wait' ? `<span style="color:var(--m3-primary);font-weight:700;">(${ddayStr(p.date)})</span>` : ''}</div>
          <div class="sub" style="font-size:12.5px;margin:3px 0;">📄 결과: <b>${p.result}</b> ${st.key === 'result_wait' || st.key === 'test_wait' || st.key === 'reg_ing' ? `<span style="color:var(--m3-primary);font-weight:700;">(${ddayStr(p.result)})</span>` : ''}</div>
    ...
    <div class="app-card">
      <b style="font-size:13px;color:var(--m3-primary);">💻 IBT (컴퓨터 시험, 한국)</b>
