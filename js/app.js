...
function viewHome() {
  ...
  return `
    ...
    <div class="app-card filled">
      <b style="font-size:13px;color:var(--ios-blue);">난이도별</b>
      ${acc.byLevel.length
    ...
    <div class="app-card elevated"><div class="row" onclick="go('schedule')" style="cursor:pointer;">
      <div><b>🗓 TOPIK 시험 일정</b><div class="sub">나라별 시험일 · 접수기간 · 결과 발표</div></div><span style="color:var(--ios-green);">→</span></div></div>
    <div class="app-card elevated"><div class="row" onclick="go('learn')" style="cursor:pointer;">
      <div><b>📚 Weak-spot review</b><div class="sub">Vocab &amp; grammar based on your mistakes</div></div><span style="color:var(--ios-green);">→</span></div></div>
    <div class="app-card elevated"><div class="row" onclick="go('wrong')" style="cursor:pointer;">
      <div><b>📓 Wrong answer notes</b><div class="sub">${lsGet(LS.wrong, []).length} recent misses · type-wise breakdown</div></div><span style="color:var(--ios-green);">→</span></div></div>
  `;
}
  ...
  return `
    <div class="app-card">
      ...
      <div style="margin:8px 0;">
        ${isAI
          ? '<span style="font-size:11px;color:var(--ios-green);font-weight:800;">✨ Generated by AI — unique to you</span>'
...
function viewDailyResult() {
  ...
  return `
    <div class="app-card big-cta">
      <div class="cta-ico">🎯</div>
      <h2 style="font-size:22px;color:var(--ios-label);margin-bottom:6px;">Daily 10 완료!</h2>
      <p class="sub">${r.total}문항 중 ${r.correct + r.wrong}문항 답변</p>
      <div style="margin:18px 0;">
        <div style="font-size:15px;color:var(--ios-secondary-label);">나의 예상 점수</div>
        ...
        <div style="margin-top:16px;padding:12px;background:#f0f3fa;border-radius:12px;">
          <div style="font-size:13px;color:var(--ios-secondary-label);margin-bottom:6px;">${est.passLabel} · ${passTarget}</div>
          <div style="font-size:30px;font-weight:800;color:var(--ios-green);">${APP.level === 'I' ? est.passI : est.passII}%</div>
          <div style="font-size:12px;color:var(--ios-secondary-label);margin-top:4px;">TOPIK ${APP.level === 'I' ? 'II' : 'I'} 전환 시 ${APP.level === 'I' ? est.passII : est.passI}%</div>
        </div>
      </div>
      <div class="stat-row">
        <div class="stat-box"><b style="color:var(--ios-green);">${r.correct}</b><span>✓ 정답</span></div>
        <div class="stat-box"><b style="color:var(--ios-red);">${r.wrong}</b><span>✗ 오답</span></div>
...
function viewWrong() {
  ...
  return `
    <div class="sec-h"><h2>📊 평균 정답률</h2><span class="sub">전체 ${acc.overall}%</span></div>
    <div class="app-card">
      <b style="font-size:13px;color:var(--ios-blue);">유형별 평균 정답률</b>
    ...
    <div class="app-card">
      <b style="font-size:13px;color:var(--ios-blue);">난이도별 평균 정답률</b>
  ...
  return `
    ...
    <div class="sec-h"><h2>📅 달력 보기</h2>
      <span style="display:flex;gap:6px;">
        ...
        <span style="font-size:13px;font-weight:800;color:var(--ios-blue);">${calMonth.slice(0,4)}년 ${+calMonth.slice(5,7)}월</span>
        <button class="btn btn-ghost btn-sm" onclick="scheduleMonth(1)">▶</button>
      </span>
    </div>
    <div class="app-card">
    ...
    ${rows.length ? rows.map(p => {
      ...
      return `
        <div class="app-card" style="${st.key === 'done' ? 'opacity:.5;' : ''}">
          ...
          <div class="sub" style="font-size:12.5px;margin:3px 0;">🖥 접수: <b>${p.reg}</b> ${reg && st.key !== 'done' && st.key !== 'result_wait' ? `<span style="color:var(--ios-blue);font-weight:700;">(${ddayStr(reg.end)})</span>` : ''}</div>
          <div class="sub" style="font-size:12.5px;margin:3px 0;">✏️ 시험: <b>${p.date}</b> ${st.key !== 'done' && st.key !== 'result_wait' ? `<span style="color:var(--ios-blue);font-weight:700;">(${ddayStr(p.date)})</span>` : ''}</div>
          <div class="sub" style="font-size:12.5px;margin:3px 0;">📄 결과: <b>${p.result}</b> ${st.key === 'result_wait' || st.key === 'test_wait' || st.key === 'reg_ing' ? `<span style="color:var(--ios-blue);font-weight:700;">(${ddayStr(p.result)})</span>` : ''}</div>
    ...
    <div class="app-card">
      <b style="font-size:13px;color:var(--ios-blue);">💻 IBT (컴퓨터 시험, 한국)</b>
